...
import { deleteContactById } from 'apiSdk/contacts';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'contact', 'product'],
      }),
  );

  const contactHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('contact', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/contacts/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.contact?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/contacts/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.phone}</Td>
