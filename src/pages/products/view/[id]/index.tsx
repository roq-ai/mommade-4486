...
import { deleteFlavorById } from 'apiSdk/flavors';
...
function ProductViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface>(
    ...
    () =>
      getProductById(id, {
        relations: ['organization', 'flavor'],
      }),
  );

  const flavorHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('flavor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Flavors
                    </Text>
                    <NextLink passHref href={`/flavors/create?product_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.flavor?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/flavors/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
